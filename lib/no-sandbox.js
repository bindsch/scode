...
const SHELL_KEYWORD_TOKENS = new Set(['then', 'do', 'done', 'fi', 'else', 'esac']);
  ...
  while (i < tokens.length) {
    ...
    if (WRAPPERS.has(token)) {
      ...
      while (i < tokens.length) {
        ...
        if (!consumedPositional && shouldConsumeWrapperPositional(token, wTok)) {
          i += 1;
...
function findShellDashCArgToken(tokens, shellIndex, segmentEnd) {
  ...
  while (j < segmentEnd) {
    ...
    if (isShellDashCFlag(arg) && j + 1 < segmentEnd) {
      // shell -c takes the immediate next token as the command string.
      // `shell -c -- "cmd"` executes "--" as the command string; "cmd" is $0.
      return tokens[j + 1];
    }
    if (arg === '--') {
      j += 1;
  ...
  try {
    ...
    if (isShellSpawn) {
      if (typeof command === 'string') {
        ...
        if (merged.merged) {
          normalized.args = merged.args;
        }
        const injectedCommand = injectNoSandboxCommand(command);
        // If we already rewrote a shell command string, avoid patching args again
        // (can duplicate --no-sandbox in shell:true command+args form).
        shouldPatchArgs = injectedCommand === command;
